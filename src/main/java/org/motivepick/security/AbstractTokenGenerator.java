...
abstract class AbstractTokenGenerator(private val userService:UserService, private val tokenService:JwtTokenService,
    ...
    String generateJwtToken(String code, String redirectUri, String language) {
        var accessToken = requestAccessToken(code, redirectUri)
        var profile = requestProfile(accessToken)
    ...
    private TokenResponse requestAccessToken(String code, String redirectUri) {
        var uri = UriComponentsBuilder.fromUriString(config.accessTokenUri)
            .queryParam("client_id", config.clientId)
            .queryParam("client_secret", config.clientSecret)
            .queryParam("code", code)
            .queryParam("redirect_uri", redirectUri).build().toUri();
        return httpClient.getForObject(uri, TokenResponse.class)
